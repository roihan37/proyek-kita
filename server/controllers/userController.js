...
module.exports = {
    createUser: async (req, res, next) => {
        try {
            ...
            // create and save
            const newUser = new User({ businessType, companyLocation, companyName, name, email, phoneNumber, password : hashPassword })
            await newUser.save();
    ...
    forgotPassword: async (req, res, next) => {
        try {
            ...
            // create token and expires date and then save
            ...
            const mailOptions = {
                ...
                html:
                  ...
                  Please click the button bellow\n\n<br><br>` +
                    `<a class="btn" style="font-size:16px;text-decoration:none;background: #0043A1;color:white;border-radius:10px;padding-left:18px;padding-right:18px;padding-top:9px;padding-bottom:9px;" 
                  href="http://localhost:8080/users/resetPswd/${token}"><b>RESET MY PASSWORD</b></a><br><br>` +
            ...
            transporter.sendMail(mailOptions, (err) => {
                ...
                return res.status(200).send({ message: "Email sent", token });
            });
    ...
    resetPassword: async (req, res, next) => {
        try {
            ...
            if (error.name === "badRequest") {
                res.status(400).json({ message: "Email / Password is required" })
            } else if (error.name === "UnauthorizedToken") {
                res
                    .status(404)
